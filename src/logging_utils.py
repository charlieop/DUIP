"""Run-level logging: console + per-step JSONL + Weights & Biases.

The :class:`RunLogger` is the single entry point used by ``train.py`` and
``evaluate.py``. It is deliberately tolerant: if ``wandb`` is not installed
or ``logging.wandb.enabled`` is ``False`` the wandb calls become no-ops, so
training keeps running unattended (e.g. on a fresh box where the user
hasn't done ``wandb login`` yet).
"""
...
# ----------------------------------------------------------------------------
# Optional wandb import (logged once if missing).
# ----------------------------------------------------------------------------
...
def _flatten(d: Dict[str, Any], parent: str = "", sep: str = ".") -> Dict[str, Any]:
    """Flatten a nested dict for wandb-config compatibility."""
    out: Dict[str, Any] = {}
    for k, v in d.items():
...
def collect_hardware_info(extra: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Return a dict describing the host / GPU / library versions."""
    info: Dict[str, Any] = {
...
@dataclass
class _Throughput:
    """Sliding-window samples/sec + tokens/sec accumulator."""
...
class RunLogger:
    """Aggregates console / file / JSONL / wandb logging for one run."""
    ...
    def __init__(
        ...
        *,
        ...
        # Console + per-run text log via the existing helper.
        self.logger = get_logger(f"{run_kind}.{self.run_name}", str(self.run_dir))

        # JSONL metrics file.
        self._jsonl_fp = None
        if self.jsonl_enabled:
            self._jsonl_fp = open(self.run_dir / "metrics.jsonl", "w",
                                  encoding="utf-8")

        # Wandb init (best-effort).
        self.wandb_enabled = (
        ...
        if self.wandb_enabled:
            try:
                ...
                # `reinit=True` is deprecated since wandb 0.18; use the
                # newer string form when available, fall back otherwise.
                try:
                    self.wandb_run = wandb.init(
                        reinit="finish_previous", **init_kwargs,
                    )
                except (TypeError, ValueError):
                    self.wandb_run = wandb.init(reinit=True, **init_kwargs)
            except Exception as e:  # pragma: no cover
                self.logger.warning("wandb.init failed (%s); continuing without wandb.", e)
    ...
    def update_throughput(self, n_samples: int, n_tokens: int, dt: float) -> Dict[str, float]:
    ...
    def update_loss_ema(self, loss: float) -> float:
        if self._loss_ema is None:
            ...
            self._loss_ema = (
                (1.0 - self._ema_alpha) * self._loss_ema + self._ema_alpha * float(loss)
            )
    ...
    def log_step(
        ...
        *,
        ...
        """Write one row of step-level metrics to console + JSONL + wandb."""
        self._step = step
        ...
        compact = " ".join(
            f"{k.split('/', 1)[-1]}={_fmt(v)}" for k, v in flat.items()
        ...
        self._write_jsonl({"event": "step", "step": step,
    ...
    def log_epoch(self, metrics: Dict[str, Any], epoch: int,
        ...
        self.logger.info("epoch=%d | %s", epoch, " ".join(
            f"{k.split('/', 1)[-1]}={_fmt(v)}" for k, v in flat.items()
    ...
    # Allow `with RunLogger(...) as log:` usage.
