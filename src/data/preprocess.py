...
def build_sessions(
    ...
    *,
    ...
    logger.info(
        "After title-join: %d reviews, %d unique items.",
        len(reviews),
    ...
    sessions = [
        s for s in sessions if user_session_counts[s["user_id"]] >= min_user_sessions
    ]
    logger.info(
        "After min_user_sessions=%d filter: %d sessions.",
        min_user_sessions,
        len(sessions),
    )
    ...
    stats = {
        ...
        "n_val": len(val),
        "n_test": len(test),
        "avg_session_len": round(sum(len(s["items"]) for s in sessions) / max(n, 1), 3),
    }
